...
[Franka Panda demo](https://siruiu.github.io/Mujoco-Franka/) | [Load the high-detail Franka XML](https://siruiu.github.io/Mujoco-Franka/?model=local_model/franka_emika_panda/scene.xml)
...
- Install frontend dependencies (Three.js is served locally):
  - `npm install`
...
- Public GitHub Pages demo (high-detail Franka Panda, loaded by default):
  - `https://siruiu.github.io/Mujoco-Franka/`
- Load the high-detail Franka XML explicitly:
...
- `dist/3.5.0/` includes the Forge runtime (`mujoco.js` and `mujoco.wasm`) needed for this local demo. Other versions require a forge `dist/<ver>/` bundle.
- Set the dist base via `forgeBase=` (recommended) or `window.__FORGE_DIST_BASE__` (must be set before the main module runs).
- Default local dist base is `/forge/dist/{ver}/`, where `{ver}` comes from `site_config.js` (`globalThis.PLAY_VER`) or `ver=...`. Hosted deployments can set `forgeBase=` to a hosted bundle.

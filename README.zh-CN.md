...
- `dist/3.5.0/` 已包含本地演示所需的 Forge 运行时（`mujoco.js` 和 `mujoco.wasm`）；其他版本仍需对应的 forge `dist/<ver>/` bundle。
- 通过 `forgeBase=`（推荐）或 `window.__FORGE_DIST_BASE__` 指定 dist base（必须在主模块运行前设置）。
- 本地默认 dist base 为 `/forge/dist/{ver}/`，其中 `{ver}` 来自 `site_config.js`（`globalThis.PLAY_VER`）或 URL 参数 `ver=...`。线上部署可通过 `forgeBase=` 指向托管的 bundle。
